...
| P — Prática guiada | 30 min | Newsletter semanal da área do aluno no Claude Code: um agente acionado sob demanda, sem agendamento, que busca o que saiu na semana anterior nas fontes listadas pelo próprio aluno e gera a edição em Markdown. 5 min de setup, 2 de linha de base com `/context`, 5 de configuração das fontes, 8 de execução com um subagente por fonte, 4 de verificação dos links, 3 de troca de tom via `references/` e 3 de debrief. Kit em [`pratica/`](../pratica/), plano do instrutor em [pratica-guiada.md](pratica-guiada.md). |
